...
function ISSelfRestrain:update()
    ...
    if not Restrain.isRestrainedByItem(self.character) then
        self:setActionAnim("Prisonner_Surrender")--surrender emote animation by Arendameth
        if Restrain.Verbose then print ("ISSelfRestrain:update surrender emote request restrainingItem= "..tostring(Restrain.isRestrainedByItem(self.character) and "yes" or "no")) end
    end
...
function ISSelfRestrain:forceStop()
    --if PlayerHitReactionState or PlayerHitReactionPVPState then do not leave, or prepare to reactivate.
    
    if self.character:getModData().isRestrained and self.character:isCurrentState(PlayerHitReactionState.instance()) or self.character:isCurrentState(PlayerHitReactionPVPState.instance()) then
        if Restrain.Verbose then print ("ISSelfRestrain:forceStop rejected on hit reaction.") end
    else
...
function ISSelfRestrain:stop()
    ...
    if self.character:getModData().isRestrained and self.character:isCurrentState(PlayerHitReactionState.instance()) or self.character:isCurrentState(PlayerHitReactionPVPState.instance()) then
        if Restrain.Verbose then print ("ISSelfRestrain:stop rejected on hit reaction.") end
    else
