...
function Restrain.isSurrendering(player)
    ...
    if ISSelfRestrain.instance then
        --occures when trying some stuff (unidentified) after being hit.
        if Restrain.Verbose then print ("Restrain.isSurrendering no ISSelfRestrain in queue while instance exists. Smells fishy !"); end
        if not currentAction then
            if Restrain.Verbose then print ("Restrain.isSurrendering probably got a bugged action, cleared queue log TODO correct that case. inhibit ISTimedActionQueue.onTick ?"); end
